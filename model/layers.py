...
__all__ = ['forward_hook', 'ReLU', 'BatchNorm2d', 'MaxPool2d',
           'AdaptiveAvgPool2d', 'Conv2d', 'testfun','PReLU', "PixelShuffle", "Upsample", "LeakyReLU","LayerNorm"]
...
class Conv2d(nn.Conv2d):
    def IR(self, I):
        ...
        if X.shape[1] == 3:
            B = X * 0 + \
                torch.min(torch.min(torch.min(X, dim=1, keepdim=True)[0], dim=2, keepdim=True)[0], dim=3, keepdim=True)[
                    0]
            H = X * 0 + \
                torch.max(torch.max(torch.max(X, dim=1, keepdim=True)[0], dim=2, keepdim=True)[0], dim=3, keepdim=True)[
                    0]
            ...
            I = X * torch.autograd.grad(Y1, X, I)[0] - B * torch.autograd.grad(Y2, B, I)[0] - H * \
                torch.autograd.grad(Y3, H, I)[0]

        else:

            Y1 = F.conv2d(positive_input, positive_weight, bias=None, stride=self.stride, padding=self.padding,
                          groups=self.groups)
            Y2 = F.conv2d(nagative_input, nagative_weight, bias=None, stride=self.stride, padding=self.padding,
                          groups=self.groups)

            I = divide_with_zero(I, Y1 + Y2)
            I = positive_input * torch.autograd.grad(Y1, positive_input, I)[0] + nagative_input * \
                torch.autograd.grad(Y2, nagative_input, I)[0]
            print("I", I.size())
